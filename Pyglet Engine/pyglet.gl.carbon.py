...
# Embedded file name: C:\TeamCity\buildAgent\work\dc8eb0b1d2cf198a\Main\client\standalone\build\pyi.win32\run_obfuscated\out00-PYZ.pyz\pyglet.gl.carbon
...
class CarbonConfig(Config):
    ...
    def match(self, canvas):
        ...
        if not pformat:
            ...
            return [
             CarbonCanvasConfig(canvas, self, pformat, self)]
            return


class CarbonCanvasConfig(CanvasConfig):
    _attribute_ids = {'double_buffer': agl.AGL_DOUBLEBUFFER, 
    ...
    def compatible(self, canvas):
        return isinstance(canvas, CarbonCanvas) or isinstance(canvas, CarbonFullScreenCanvas)
...
class CarbonContext(Context):
    ...
    def attach(self, canvas):
        ...
        if isinstance(canvas, CarbonFullScreenCanvas):
            ...
            agl.aglSetFullScreen(self._context, canvas.width, canvas.height, canvas.screen._refresh_rate, 0)
        else:
            agl.aglSetDrawable(self._context, cast(canvas.drawable, agl.AGLDrawable))
    ...
    def flip(self):
        ...
        _aglcheck()
# okay decompiling out\pyglet.gl.carbon.pyc
