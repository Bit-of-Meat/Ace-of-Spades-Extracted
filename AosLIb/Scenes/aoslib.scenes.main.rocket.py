...
# Embedded file name: C:\TeamCity\buildAgent\work\dc8eb0b1d2cf198a\Main\client\standalone\build\pyi.win32\run_obfuscated\out00-PYZ.pyz\aoslib.scenes.main.rocket
...
class Rocket(Entity):
    ...
    def __init__(self, scene, *arg, **kw):
        ...
        if self.player and self.player.tool_id == A308:
    ...
    def set_position(self, x, y, z):
        if self.world_object:
            ...
            if self.rocket_type == 'rocket_turret' and not self.shoot_sound_played:
                self.scene.media.play_pitched(A2825, pos=(x, y, z), zone=media.IN_WORLD_AUDIO_ZONE)
    ...
    def delete(self):
        ...
        if self.rocket_type == 'rpg':
            if pos.z >= A2214 - 1:
                self.scene.media.play_pitched(A2763, pos=(pos.x, pos.y, pos.z), zone=media.IN_WORLD_AUDIO_ZONE)
            else:
                self.scene.media.play_pitched(A2762, pos=(pos.x, pos.y, pos.z), zone=media.IN_WORLD_AUDIO_ZONE)
        elif self.rocket_type == 'rocket_turret':
            if pos.z >= A2214 - 1:
                self.scene.media.play_pitched(A2767, pos=(pos.x, pos.y, pos.z), zone=media.IN_WORLD_AUDIO_ZONE)
            else:
                self.scene.media.play_pitched(A2766, pos=(pos.x, pos.y, pos.z), zone=media.IN_WORLD_AUDIO_ZONE)
        if self.sound:
            self.sound.close()
        super(Rocket, self).delete()
        return
# okay decompiling out\aoslib.scenes.main.rocket.pyc
