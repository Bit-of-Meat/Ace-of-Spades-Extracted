...
# Embedded file name: C:\TeamCity\buildAgent\work\dc8eb0b1d2cf198a\Main\client\standalone\build\pyi.win32\run_obfuscated\out00-PYZ.pyz\twisted.internet.base
...
class ReactorBase(object):
    ...
    def callWhenRunning(self, _callable, *args, **kw):
        if self.running:
            ...
            return self.addSystemEventTrigger('after', 'startup', _callable, *args, **kw)
    ...
    def callLater(self, _seconds, _f, *args, **kw):
        tple = DelayedCall(self.seconds() + _seconds, _f, args, kw, self._cancelCallLater, self._moveCallLaterSooner, seconds=self.seconds)
    ...
    def _checkProcessArgs(self, args, env):
        ...
        def argChecker(arg):
            if isinstance(arg, unicode):
                ...
                warnings.warn('Argument strings and environment keys/values passed to reactor.spawnProcess should be str, not unicode.', category=DeprecationWarning, stacklevel=4)
            if isinstance(arg, bytes) and '\x00' not in arg:
                return arg
            else:
                return
...
__all__ = []
# okay decompiling out\twisted.internet.base.pyc
